...
do
    ...
    ${YCSB_HOME}/bin/ycsb load basic -P ${YCSB_HOME}/workloads/workloadb -P ./ycsb_workload_settings/$setting > $setting.load
    echo generateing $setting.run, the lookup queries used before benchmark
    ${YCSB_HOME}/bin/ycsb run basic -P ${YCSB_HOME}/workloads/workloadb -P ./ycsb_workload_settings/$setting > $setting.run

    echo using predefined workloadc to create transaction records for $setting with reads only 
    echo generateing $setting.load, the insertions used before benchmark
    ${YCSB_HOME}/bin/ycsb load basic -P ${YCSB_HOME}/workloads/workloadc -P ./ycsb_workload_settings/$setting > $setting.load
    echo generateing $setting.run, the lookup queries used before benchmark
    ${YCSB_HOME}/bin/ycsb run basic -P ${YCSB_HOME}/workloads/workloadc -P ./ycsb_workload_settings/$setting > $setting.run
done
